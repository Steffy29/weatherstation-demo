...
void drawTempIntern(OLEDDisplay *display, OLEDDisplayUiState* state, int16_t x, int16_t y);
...
void drawDateTime(OLEDDisplay *display, OLEDDisplayUiState* state, int16_t x, int16_t y) {
  ...
  // heure d'été
  sprintf_P(buff, PSTR("%02d:%02d:%02d"), (timeInfo->tm_hour-1+24)%24, timeInfo->tm_min, timeInfo->tm_sec);
...
void drawHeaderOverlay(OLEDDisplay *display, OLEDDisplayUiState* state) {
  ...
  timeInfo = localtime(&now);
  char buff[14];
  sprintf_P(buff, PSTR("%02d:%02d"), (timeInfo->tm_hour-1+24)%24, timeInfo->tm_min);
...
void drawTempIntern(OLEDDisplay *display, OLEDDisplayUiState* state, int16_t x, int16_t y) {
  display->setFont(ArialMT_Plain_10);
  display->setTextAlignment(TEXT_ALIGN_CENTER);
  display->drawString(64 + x, y, "Intérieur");

  display->setFont(ArialMT_Plain_10);
  display->setTextAlignment(TEXT_ALIGN_CENTER);
  String tempInt = "Temp : " + String(temp) + "°C";
  String humidity = "Humi : " + String(humi) + "%"; 
  display->drawString(30 + x, 10 + y, tempInt);
  display->drawString(90 + x, 10 + y, humidity);

  String light = "Light : " + String(tempLight);
  float atom = tempAtom / 100;
  String pressure = "Pressure : " + String(atom) + " hPa";
  display->drawString(60 + x, 25 + y, light);
  display->drawString(60 + x, 38 + y, pressure);
}
